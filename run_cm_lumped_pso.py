...
version of the conceptual models using Particle Swarm Optimisation (PSO) algorithm.

Note: this code is inspired from that of Sopan Patil
"""
...
from conceptual_models.lumped import (ExphydroModel, ExphydroParameters, ABCDModel, ABCDParameters,
                                      HBVModel, HBVParameters, WBModel, WBMParameters, FLEXAModel, FLEXParameters)
...
# Load meteorological and observed flow data  # numpy array
...
params = [FLEXParameters() for j in range(npart)]

# Initialise the model by loading its climate inputs
model = FLEXAModel(P, PET, T)
